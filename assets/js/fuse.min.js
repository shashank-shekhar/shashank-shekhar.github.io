	...
	(t = function () {
		...
		function T(e, t, n) {
			...
			for (
				...
				if (((x = Math.min(_, x)), (w = m + k), S)) for (var O = 0; O < k; ) ((L[m + O] = 1), (O += 1));
			}
			w = -1;
			for (var j = [], A = 1, I = k + M, C = 1 << (k - 1), E = 0; E < k; E += 1) {
				for (var F = 0, R = I; F < R; )
					(N(t, { errors: E, currentLocation: b + R, expectedLocation: b, distance: a, ignoreLocation: p }) <= x
						? (F = R)
						: (I = R),
				...
				for (var D = T; D >= P; D -= 1) {
					...
					if (
						...
						z[D] & C &&
							(A = N(t, { errors: E, currentLocation: K, expectedLocation: b, distance: a, ignoreLocation: p })) <= x)
					) {
						if (((x = A), (w = K) <= b)) break;
						P = Math.max(1, 2 * b - w);
					}
				}
				if (N(t, { errors: E + 1, currentLocation: b, expectedLocation: b, distance: a, ignoreLocation: p }) > x) break;
				j = z;
			}
			...
			if (S) {
				var J = (function () {
					for (
						var e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : [],
							t = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : $.minMatchCharLength,
							n = [],
							r = -1,
							i = -1,
							o = 0,
